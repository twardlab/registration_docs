...
from step3_help import load_swc, backup_unpickle, Unpickler, ClassHack, str_to_list, get_xyzoff, get_target_ids, atlas_to_cp_mask, compute_all_dice_scores, plot_all_slices, compute_all_hausdorff_scores, ternary_to_boundary
...
def main():
    ...
    parser.add_argument('-mask_paths', type = str, default = None, nargs = '*', help = 'The file paths to any manual segmentations to be used for registration performance quantification')
    parser.add_argument('-mask_inds', type = int, default = None, nargs = '*', help = 'The index of low_img_paths which corresponds to the respective mask_paths')
    ...
    # Determine if registration performance quantifictaion will beperformed
    if mask_paths == None:
        reg_pq = False
    else:
        reg_pq = True
    ...
    set_v_to_zero = False
    files_all = [] 
    cp_acb_region_ids = get_target_ids(ontology_name)
    ...
    for low_ind in all_slice_idx: 
        ...
        # If there are any manual segmentations for pq AND there is a manual segmentation for the current slice
        if reg_pq and low_ind in mask_inds:
            # Extract the manually segmented mask which corresponds to the current slice
            curr_mask_path = mask_paths[mask_idx]
            mask_idx += 1
            curr_mask = nib.load(curr_mask_path)
            curr_mask = curr_mask.get_fdata()

            # Convert the integer-valued atlas segmentation to a ternary segmentation
            atlas_mask = atlas_to_cp_mask(Stformed,cp_acb_region_ids)
            num_slices = np.shape(atlas_mask)[0]

            # Compute the dice score for the cp and the whole brain (WB) b/t the atlas and manual segmentations
            dice_cp, dice_wb = compute_all_dice_scores(atlas_mask, curr_mask)
            print(f'dice(cp): {dice_cp:.4f}, dice(wb): {dice_wb:.4f}')
            plot_all_slices(atlas_mask, curr_mask, image_low_files[low_ind], outdir, f'overlap_{brain}_{image_low_key}_reg_pq.jpg')

            # Extract the boundary of each ternary mask
            atlas_cp, atlas_wb = ternary_to_boundary(atlas_mask)
            manual_cp, manual_wb = ternary_to_boundary(curr_mask)

            # fig10, axs10 = plt.subplots(num_slices,2,layout='constrained')
            # fig10.suptitle('Atlas Segmenation | Manual Segmentation')
            # for idx in range(num_slices):
            #     axs10[idx,0].imshow(atlas_wb[idx,:,:])
            #     axs10[idx,0].imshow(atlas_cp[idx,:,:], alpha = 0.4)
            #     axs10[idx,1].imshow(manual_wb[idx,:,:])
            #     axs10[idx,1].imshow(manual_cp[idx,:,:], alpha = 0.4)
            # fig10.set_size_inches(num_slices,num_slices*2)
            # fig10.savefig(os.path.join(outdir,f'{brain}_{image_low_key}_boundaries.jpg'))

            # Get coordinates of voxels for each boundary 
            atlas_cp_x, atlas_cp_y, atlas_cp_z = np.where(atlas_cp > 0)
            atlas_bound_cp = XI[np.array([0,1,2])[:,None],atlas_cp_x,atlas_cp_y,atlas_cp_z]

            atlas_wb_x, atlas_wb_y, atlas_wb_z = np.where(atlas_wb > 0)
            atlas_bound_wb = XI[np.array([0,1,2])[:,None],atlas_wb_x,atlas_wb_y,atlas_wb_z]

            manual_cp_x, manual_cp_y, manual_cp_z = np.where(manual_cp > 0)
            manual_bound_cp = XI[np.array([0,1,2])[:,None],manual_cp_x,manual_cp_y,manual_cp_z]

            manual_wb_x, manual_wb_y, manual_wb_z = np.where(manual_wb > 0)
            manual_bound_wb = XI[np.array([0,1,2])[:,None],manual_wb_x,manual_wb_y,manual_wb_z]

            # Compute the Hausdorff distance between the atlas and manual segmentation
            hausdorff_cp, h50_cp, h95_cp, all_dist_cp = compute_all_hausdorff_scores(atlas_bound_cp, manual_bound_cp)
            print(f'CP: max {hausdorff_cp}, 50th {h50_cp}, 95th {h95_cp}')
            
            hausdorff_wb, h50_wb, h95_wb, all_dist_wb = compute_all_hausdorff_scores(atlas_bound_wb, manual_bound_wb)
            print(f'WB: max {hausdorff_wb}, 50th {h50_wb}, 95th {h95_wb}')

            # plot_all_slices(atlas_cp, manual_cp, image_low_files[low_ind], outdir, f'cp_bounds_{brain}_{image_low_key}.jpg')
            # plot_all_slices(atlas_wb, manual_wb, image_low_files[low_ind], outdir, f'wb_bounds_{brain}_{image_low_key}.jpg')
            plot_all_slices(np.logical_or(atlas_wb, atlas_cp), np.logical_or(manual_wb, manual_cp), image_low_files[low_ind], outdir, f'all_bounds_{brain}_{image_low_key}.jpg')
            
            

            np.savez(os.path.join(outdir,f'all_dist_cp_{brain}_{image_low_key}.npz'),all_dist = all_dist_cp)
            np.savez(os.path.join(outdir,f'all_dist_wb_{brain}_{image_low_key}.npz'),all_dist = all_dist_wb)
